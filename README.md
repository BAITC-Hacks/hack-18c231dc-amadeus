...
1. Установите зависимости по разделу [«Установка»](#установка): Python-окружение `.venv`
   и `npm ci`.
2. Терминал 1, в активированном `.venv`:
   `uvicorn api.main:app --host 127.0.0.1 --port 8000`.
3. Терминал 2: `npm run dev`, затем откройте http://localhost:5173/.
4. В интерфейсе нажмите «Пример» → «Рассчитать» → «Объяснить».

Ключ OpenAI не нужен: без него объяснение строится по шаблону и помечено «Шаблон».

## Что видит пользователь

1. Выбирает 5 мер из каталога, а для районных мер ещё и район. Индикатор показывает,
   сколько осталось от бюджета.
2. Нажимает «Рассчитать». Интерфейс не даёт добавить меру дороже остатка бюджета, а
   сервер проверяет все правила. Если набор их нарушает, интерфейс показывает причины
   от сервера и не показывает Score этого набора.
3. Для допустимого набора видит Score из 100 и его изменение за сценарий, занятый
   бюджет и резерв, а по каждому району — оценку и показатели после мер с изменениями.
   Для примера из датасета: Score около 56,5 при бюджете 95 из 100 (точное значение
   56.54307 — в ответе API).
4. До запроса объяснения блок «AI-анализ сценария» показывает короткий локальный разбор
   по числам сервера с меткой «Local Analysis». Нажимает «Объяснить» и получает сильные
   стороны, риски, последствия и рекомендации. С ключом OpenAI разбор делает AI-агент, ответ идёт около 30 с. Агент сам
   вызывает расчётный движок и, например, предлагает заменить M5 в Сарыарке на M3 в Нуре
   (Score 57,21). Без ключа объяснение строится по шаблону.

## Содержание

- [Быстрый старт](#быстрый-старт)
- [Что видит пользователь](#что-видит-пользователь)
- [Проверка основного сценария без ключа](#проверка-основного-сценария-без-ключа)
- [Требования ТЗ → где реализовано → как проверить](#требования-тз--где-реализовано--как-проверить)
- [Архитектура](#архитектура)
- [AI-агент (/api/explain)](#ai-агент-apiexplain)
- [Технологии](#технологии)
- [Зависимости и системные требования](#зависимости-и-системные-требования)
- [Установка](#установка)
- [Запуск](#запуск)
- [Параметры окружения](#параметры-окружения)
- [HTTP API](#http-api)
- [Движок](#движок)
- [Оригинальность и потенциал](#оригинальность-и-потенциал)
- [Известные ограничения](#известные-ограничения)
- [Сторонние компоненты и лицензии](#сторонние-компоненты-и-лицензии)
- [Как использовались AI-инструменты](#как-использовались-ai-инструменты)
- [Команда и вклад](#команда-и-вклад)
- [Подготовка до хакатона](#подготовка-до-хакатона)
...
| 7. Интерфейс | `npm run dev`, http://localhost:5173/ → «Пример» → «Рассчитать» → «Объяснить» | Те же Score и причины ошибок, что в шагах 2–5; после «Объяснить» без ключа метка «Шаблон», с ключом — «AI Generated» |
...
| `src/` | React-интерфейс. Формулы Score и валидатора в браузере нет: Score, показатели и ошибки приходят из API. Браузер только суммирует цены выбранных мер для индикатора бюджета и до запроса объяснения показывает локальный разбор по числам сервера с меткой «Local Analysis» |
...
2. `POST /api/explain` с `examples/example.json` (или «Объяснить» в
...
- В интерфейсе метка объяснения — «AI Generated» или «Шаблон»; список вызванных
  инструментов (`tools_called`) есть в ответе `/api/explain`, но интерфейс его не
  показывает. До нажатия «Объяснить» под заголовком «AI-анализ сценария» виден
  локальный разбор («Local Analysis»), а не ответ модели.
...
- **lMakEl (Малика Ерболқызы):** план и зоны, полный перебор
